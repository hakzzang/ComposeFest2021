...
fun TodoScreen(
    ...
    Column {
        TodoItemInputBackground(elevate = true, modifier = Modifier.fillMaxWidth()) {
            TodoItemInputEntry(onItemComplete = onAddItem)
        }
        LazyColumn(
...
private fun TodoActivityScreen(todoViewModel: TodoViewModel) {
    TodoScreen(
        ...
        onEditItemChanged = todoViewModel::onEditItemChanged,
...
fun TodoItemInputEntry(onItemComplete: (TodoItem) -> Unit) {
    ...
    val (icon, onIconChanged) = remember { mutableStateOf(TodoIcon.Default) }
    val iconVisible = text.isNotBlank()
    val submit = {
        if (text.isNotBlank()) {
            onItemComplete(TodoItem(text, icon))
            onTextChanged("")
            onIconChanged(TodoIcon.Default)
        }
    }
    TodoItemInput(
        text = text,
...
@Preview
@Composable
fun PreviewTodoItemInput() = TodoItemInputEntry(onItemComplete = { })